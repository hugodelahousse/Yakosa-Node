...
import { app, promotions, users } from './setup';
...
describe('PromotionsController should be able to find lists for a specific user', () => {
  it('Responds with 200', async () => {
    const user = users[0];
    const res = await chai.request(app).get(`/promotions/for/${user.id}/`);
    expect(res).to.have.status(200);
  });
  ...
  it('Returns the proper lists', async () => {
    const user = users[0];
    const res = await chai.request(app).get(`/promotions/for/${user.id}/`);
    const userLists = user.postedPromotions;
